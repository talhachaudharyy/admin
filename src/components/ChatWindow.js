...
const ChatWindow = ({ user }) => {
  ...
  {messages.map((message, index) => (
    <div key={index} className={`flex ${message.senderId === user._id ? 'justify-end mb-4 cursor-pointer' : 'mb-4 cursor-pointer'}`}>
      {/* Receiver message on the left side */}
      ...
      {/* Sender message on the right side */}
      {message.senderId === user._id && (
        <>
